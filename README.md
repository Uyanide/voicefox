...
### 2026-08-09：键盘快捷键一致性修复（v0.3.2）

- `Ctrl+L` 在所有页面统一收藏或取消收藏当前播放中的歌曲。
- 新增可配置的 `f`，收藏或取消收藏当前选中的歌曲/歌单；历史页面支持 `Delete` 删除。
- 队列页面统一支持 `Enter` / `l` 播放，`[` / `]` 快进快退仅在队列页面生效；搜索页面用其切换音源。
- 修复搜索音源版本弹层的 `Esc` / 再次 `v` 关闭逻辑；顶层页面连续按 `Esc` 不再跳回队列。
...
以下是全局绑定；快进/快退仅在队列页面执行，搜索页面的 `[` / `]` 用于切换音源。
...
- 统一全局与页面级收藏快捷键，补充历史 `Delete`、队列 `l` 播放和搜索 `[` / `]` 音源切换。
- 修复搜索音源弹层关闭和顶层 `Esc` 导航行为。
