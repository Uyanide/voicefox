...
if systemctl is-active --quiet NetworkManager; then
    if { NetworkManager --print-config 2>/dev/null | grep -iq 'wifi\.backend.*iwd' || systemctl is-active --quiet iwd; } \
       && command -v impala >/dev/null 2>&1; then
        NET_TOOL="impala"
    elif command -v nmtui >/dev/null 2>&1; then
        NET_TOOL="nmtui"
    elif command -v nm-connection-editor >/dev/null 2>&1; then
        NET_TOOL="nm-connection-editor"
    fi
    if [[ -n "$NET_TOOL" ]]; then
...
if command -v fuzzel >/dev/null 2>&1; then
    ...
    SELECTED=$(printf "%s\n" "${OPTIONS_ARR[@]}" | rofi -dmenu -i -p "Shorin指令 > " || true)
else
    report_error "未找到可用的菜单程序（fuzzel/rofi）。"
    exit 1
fi
...
case "$SELECTED" in
    ...
    "$OPT_BLUETOOTH")
        if [[ -n "$BT_TOOL" ]]; then
            case "$BT_TOOL" in
                bluetuith|bluetui|bluetoothctl)
